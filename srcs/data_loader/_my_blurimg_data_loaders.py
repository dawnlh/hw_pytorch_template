...
from srcs.utils import utils_blurkernel_zzh, utils_deblur_kair
...
class BlurImgDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.load_psf:
            ...
            # generate random psf
            # ------- zzh codedRandomMotionBlurKernel --------
            # psfk = utils_deblur_zzh.codedRandomMotionBlurKernel(
            #     motion_len_r=self.motion_len, psf_sz=80, code=self.ce_code)
            # ------- zzh  randomBlurKernelSynthesis --------
            # psfk = utils_deblur_zzh.randomBlurKernelSynthesis(psf_sz=37)
            # ------- kair blurkernel_synthesis_zzh --------
            psfk = utils_deblur_kair.blurkernel_synthesis_zzh(h=37)
            # ------- Boracchi create_psf --------
            # max_total_length = np.random.uniform(*self.motion_len)
            # anxiety = np.random.uniform(0.004, 0.006)
            # num_samples = np.int(np.random.uniform(1500, 2000))
            # psfk = utils_deblur_zzh.create_random_psf(psf_size=64, trajSize=64, anxiety=anxiety,
            #                                           num_samples=num_samples, max_total_length=max_total_length)
...
class BlurImgDataset_all2CPU(Dataset):
    ...
    def __init__(self, img_dir=None, ce_code=None, patch_sz=256, tform_op=None, noise_type='gaussian', noise_params={'sigma': 0}, motion_len=0, load_psf_dir=None, test_mode='one2part'):
...
if __name__ == '__main__':
    ...
    img_dir = 'dataset/benchmark/Kodak24/'
    ...
    test_dataloader = get_data_loaders(
        img_dir, patch_size=None, load_psf_dir=load_psf_dir, noise_type='camera', noise_params={'sigma_beta': [0, 0.02], 'sigma_r': [0, 0.03], 'nd_factor': [1, 5], 'kc': [5, 20]},  motion_len=[25, 48], batch_size=1, num_workers=8, shuffle=False, all2CPU=False, status='test')
    ...
    for coded_blur_img_noisy, psf, sharp_img in iter_dataloader:  # val_dataloader
        k += 1
