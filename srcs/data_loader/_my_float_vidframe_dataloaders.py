...
#     ├─ ...
#
# output data: shape=[frame_num, c, h, w], dtype=float32, range=[0, 1]
# =================
...
class VideoFrame_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if noise_level > 0:
            vid = vid + np.random.normal(0, noise_level, vid.shape).astype(np.float32)
            vid = vid.clip(0, 1)

        # shape = [frame_num, c, h, w], dtype = float32, range = [0, 1]
