...
class ImagePairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if noise_level > 0:
            image_dtype = datak.dtype
            # maxv: 255/65535 for uint8/uint16
            image_maxv = np.iinfo(image_dtype).max
            datak = datak + \
                np.random.normal(0, image_maxv*noise_level, datak.shape)
            datak = datak.clip(0, image_maxv).astype(image_dtype)

         # datak/targetk: [C,H,W], uint8/uint16, noise_level:0-1
...
class ImagePairDataset_all2CPU(Dataset):
    ...
    def __getitem__(self, idx):
        # load image
        datak = self.data_imgs[idx]
        targetk = self.target_imgs[idx]
        img_sz = datak.shape
