...
def get_data_loaders(noisy_image_dir=None, gt_img_dir=None, patch_size=256, batch_size=8, tform_op=None, noise_type='gaussian', noise_params={'sigma': 0.05}, shuffle=True, validation_split=None, status='train', num_workers=8, pin_memory=False, prefetch_factor=2, all2CPU=True):
    # dataset
    if status in ['train', 'test', 'valid']:
        if all2CPU:
            dataset = ImgNoiseDataset_all2CPU(
                gt_img_dir, patch_size, tform_op, noise_type, noise_params)
        else:
            dataset = ImgNoiseDataset(
                gt_img_dir, patch_size, tform_op, noise_type, noise_params)
    elif status in ['simuexp', 'realexp']:
        dataset = ImgNoiseDataset_Exp_all2CPU(
            noisy_image_dir, gt_img_dir, patch_sz=patch_size, exp_mode=status)
    else:
