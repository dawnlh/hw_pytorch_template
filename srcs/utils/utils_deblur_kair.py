...
def warp_pad_size(img_sz, k_size):
    ...
    w = img_sz[0]
    h = img_sz[1]
    d2 = int(math.log(w + k_size[0] + 1, 2))+1
    d3 = int(math.log(w + k_size[0] + 1, 3))+1
    d5 = int(math.log(w + k_size[0] + 1, 5))+1
    d7 = int(math.log(w + k_size[0] + 1, 7))+1
    wn = np.min((2**d2, 3**d3, 5**d5, 7**d7))
    d2 = int(math.log(h + k_size[1] + 1, 2))+1
    d3 = int(math.log(h + k_size[1] + 1, 3))+1
    d5 = int(math.log(h + k_size[1] + 1, 5))+1
    d7 = int(math.log(h + k_size[1] + 1, 7))+1
...
def blurkernel_synthesis(h=37, w=None):
    ...
    # zzh: contain small bugs which may geneate kernel with size not equal to [h,w]. Use blurkernel_synthesis_zzh below
...
def blurkernel_synthesis_zzh(h=37, w=None):
...
if __name__ == '__main__':
    ...
    for k in range(20):
        psf = blurkernel_synthesis_zzh(37)
