...
def augment_img(img, prob=0.5, tform_op=["all"]):
    ...
    if "rotate" in tform_op or "all" in tform_op:
        ...
        if prob / 4 < np.random.rand() <= prob / 2:
            ...
            img = np.transpose(img[::-1, :, :][:, ::-1, :], axes=(1, 0, 2))[
                ::-1, ...
            ]  # 90
...
def augment_vid(vid, prob=0.5, tform_op=["all"]):
    ...
    if "rotate" in tform_op or "all" in tform_op:
        ...
        if prob / 4 < np.random.rand() <= prob / 2:
            ...
            vid = np.transpose(vid[:, ::-1, :, :][:, :, ::-1, :], axes=(0, 2, 1, 3))[
                :, ::-1, ...
            ]  # 90
...
def circular_padding(tensor, pad_len):
    """
    ...
    :return:
    """
    x_pad_len, y_pad_len = pad_len[0], pad_len[1]
    pad_width = (
        y_pad_len // 2,
        y_pad_len - y_pad_len // 2,
        x_pad_len // 2,
        x_pad_len - x_pad_len // 2,
    )
...
def img_blur(img, psf, noise_level=0.01, mode="circular"):
    ...
    # add Gaussian noise
    blur_noisy_img = blur_img + np.random.normal(0, noise_level, blur_img.shape)
...
def imsave(img, img_path, compress_ratio=0):
    ...
    if img_path.split(".")[-1] in ["jpg", "jpeg"]:
        cv2.imwrite(
            img_path, img, [cv2.IMWRITE_JPEG_QUALITY, (10 - compress_ratio) * 10]
        )
    elif img_path.split(".")[-1] in ["png", "PNG"]:
        cv2.imwrite(img_path, img, [cv2.IMWRITE_PNG_COMPRESSION, compress_ratio])
    else:
...
def img_matrix(imgs, n_row, n_col, margin=0):
    ...
    if len(imgs) != n:
        raise ValueError(
            "Number of images ({}) does not match "
    ...
    imgmatrix = np.zeros(
        (img_h * n_row + m_y * (n_row - 1), img_w * n_col + m_x * (n_col - 1), img_c),
        np.uint8,
    )
