...
class GaussianKernelSampler(KernelSamplerBase):
    ...
    def shift_kernel(self, kernel: np.array) -> np.array:
        ...
        wanted_center_of_mass = self.kernel_size // 2 + \
            0.5 * (1 - self.scale_factor % 2)
    ...
    def sample_kernel(self) -> np.array:
        ...
        noise_level = self.min_noise_level + \
            np.random.rand() * (self.max_noise_level - self.min_noise_level)
        ...
        raw_kernel = np.exp(-0.5 * np.squeeze(ZZ_t @
                            INV_SIGMA @ ZZ)) * (1 + noise)
...
class ShakeKernelSampler(KernelSamplerBase):
    ...
    def sample_trajectory(self) -> np.array:
        ...
        for t in range(self.num_samples - 1):
            ...
            v = (v / np.abs(v)) * self.max_total_length / \
                (self.num_samples - 1)
...
if __name__ == '__main__':
    ...
    psf_sampler = ShakeKernelSampler(
        kernel_size=64, trajectory_size=64, anxiety=0.005, num_samples=1500, max_total_length=64)
    for k in range(100):
        psf = psf_sampler.sample_kernel()
