...
def img_blur(img, psf, noise_level=0.01, mode="circular", cval=0):
    ...
    blur_img = ndimage.filters.convolve(img, psf, mode=mode, cval=cval)
...
def img_blur_torch(
    img, psf, noise_level=0.0, conv_mode="conv", pad_mode="circular", pad_value=0.0
):
    """
    ...
    for k in range(N1 * C1):
        blur_img[k // 3, k % 3] = F.conv2d(
            img_pad[k // 3, k % 3].unsqueeze(0).unsqueeze(0),
            psf[k // 3, k % 3].unsqueeze(0).unsqueeze(0),
            padding="valid",
        ).squeeze()

    # add Gaussian noise
    if noise_level > 0:
        blur_img = blur_img + torch.tensor(
            np.random.normal(0, noise_level, blur_img.shape), dtype=torch.float32
        )
    return blur_img
...
def edgetaper_np(img, kernel, n_tapers=3):
    ...
    for i in range(n_tapers):
        blurred = fftconvolve(
            pad_for_kernel_np(img, _kernel, "wrap"), kernel, mode="valid"
        )
