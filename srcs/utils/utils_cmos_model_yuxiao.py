...
if __name__ == "__main__":
    ...
    test_img_dir = '/hdd/1/zzh/project/LLDeblur/dataset/benchmark/paper_figure/gt/'

    def get_testimg(index=None, imgsize=None):
        imgpath = os.path.join(test_img_dir)
    ...
    model = CMOS_Camera({"sigma_beta": 0.02, "nd_factor": 10, "sigma_r": 1})

    k = 5
    # cleanimg = (np.ones([100,100,3]) * 180).astype(np.uint8)
    cleanimg = get_testimg(index=0)

    model = model.take_photo_P(img_source=cleanimg, kd=k/6, ka=4)
    gaussian = np.clip(cleanimg + np.random.normal(0, (100*k)
                       ** 0.5, size=cleanimg.shape), 0, 255).astype(np.uint8)
    poisson = np.clip(np.random.poisson(lam=cleanimg/k)
                      * k, 0, 255).astype(np.uint8)
    ...
    cv2.imwrite("./outputs/test/camera/gen/model.bmp", cv2.resize(cv2.cvtColor(model,
                cv2.COLOR_RGB2BGR), [500, 500], interpolation=cv2.INTER_NEAREST))
    cv2.imwrite("./outputs/test/camera/gen/poisson.bmp", cv2.resize(cv2.cvtColor(poisson,
                cv2.COLOR_RGB2BGR), [500, 500], interpolation=cv2.INTER_NEAREST))
    cv2.imwrite("./outputs/test/camera/gen/gaussian.bmp", cv2.resize(cv2.cvtColor(gaussian,
                cv2.COLOR_RGB2BGR), [500, 500], interpolation=cv2.INTER_NEAREST))
    cv2.imwrite("./outputs/test/camera/gen/clean.bmp", cv2.resize(cv2.cvtColor(cleanimg,
                cv2.COLOR_RGB2BGR), [500, 500], interpolation=cv2.INTER_NEAREST))
